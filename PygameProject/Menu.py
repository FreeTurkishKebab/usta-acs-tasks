...
#----------A second button class for no wooden image background-------------
class Button2:
    def __init__(self, x, y, width, height, text, action):
        self.rect = pygame.Rect(x, y, width, height)
        self.text = text 
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            if self.rect.collidepoint(event.pos):
                self.action()
def start_game():
    import ProjectUpdating
    open(file="ProjectUpdating.py")
...
buttons = [start_button, quit_button, LB_button, settings_button]
...
while running:
    ...
    screen.blit (cog, (0,0)) #draw the cog icon
    pygame.display.flip()
